  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
      ...
      <div className="glass" style={{ borderRadius: 18, padding: '20px 20px 10px' }}>
        <p style={{ margin: '0 0 16px', color: 'rgba(245,247,251,0.42)', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 700 }}>
          Rent Bridge
        </p>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={rentData} barCategoryGap="30%">
            <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
            <XAxis dataKey="label" tick={{ fill: 'rgba(245,247,251,0.42)', fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} tick={{ fill: 'rgba(245,247,251,0.42)', fontSize: 11 }} axisLine={false} tickLine={false} />
            <Tooltip
            ...
            />
            <Bar dataKey="value" fill="#9fb8ff" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <AreaChart data={growthData}>
            <defs>
              <linearGradient id="gv" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#9fb8ff" stopOpacity={0.25} />
                <stop offset="95%" stopColor="#9fb8ff" stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
            <XAxis dataKey="year" tick={{ fill: 'rgba(245,247,251,0.42)', fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis tickFormatter={v => `$${(v / 1e6).toFixed(1)}M`} tick={{ fill: 'rgba(245,247,251,0.42)', fontSize: 11 }} axisLine={false} tickLine={false} />
            <Tooltip
              ...
              itemStyle={{ color: '#f5f7fb' }}
            />
            <Area type="monotone" dataKey="value" stroke="#9fb8ff" fill="url(#gv)" strokeWidth={2} dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
        ...
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={noiData} barCategoryGap="30%">
            ...
            <Tooltip
              ...
              itemStyle={{ color: '#f5f7fb' }}
            />
            <Bar dataKey="value" fill="#5ee0a1" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
